...
| Auth (Register/Login) | ✅ | **Supabase Auth** (email + password + Google OAuth). AuthProvider for state management, auto session persistence. Google Sign-In via `google_sign_in` package: `AuthService.signInWithGoogle()` gets idToken from `GoogleSignIn().signIn()` and passes it to `supabase.auth.signInWithIdToken()`. Auto-creates user profile in `users` table for first-time Google users. Login and Register screens at `lib/views/auth/` with "Se connecter avec Google" buttons. |
...
| 14 | admin-dashboard/app.js + style.css | ✅ Fixed | **Admin dashboard JS/CSS bugs:** (1) `switchPage()` called `renderCategoryChart()` without arguments — chart never rendered on page tab switch. (2) `markAllNotificationsRead()` used hardcoded Arabic strings instead of `t()` — broken in French mode. (3) CSS missing `.badge-falseReport` class — `falseReport` status badge in modal had no styling. (4) Toggle switch used `right` positioning only — broken in LTR (French). **Fix:** Pass `_allReports` to `renderCategoryChart()`, added `noUnread`/`allRead` LANG keys, added `.badge-falseReport` CSS class, added `[dir="ltr"]` overrides for toggle switch. |
| 15 | auth_service.dart + auth_controller.dart + strings.xml | ✅ Fixed | **Google Sign-In échoue.** Causes multiples : (1) `GoogleSignIn()` sans `serverClientId` → `idToken` toujours null sur Android. (2) `GoogleSignIn()` sans `clientId` → crash sur web (Assertion `ClientID not set`). (3) `AuthException` de Gotrue non capturée → message générique "حدث خطأ غير متوقع". (4) Android SHA-1 absent → code 10 `DEVELOPER_ERROR`. **Fix:** (1) `serverClientId` + `clientId` passés au constructeur `GoogleSignIn()`. (2) `target/src/main/res/values/strings.xml` créé avec `default_web_client_id`. (3) `_extractErrorMessage` détecte `AuthException` directement. (4) Debug SHA-1 `62:71:E2:...` ajouté dans Google Cloud Console. |
| 16 | auth_service.dart:30-35 + AndroidManifest.xml + main.dart | ✅ Fixed | **Email confirmation redirects to localhost:3000, link expires (otp_expired).** 3 fixes : (1) `redirectTo: 'io.supabase.baligh://login-callback'` ajouté à `signUp()`. (2) Intent-filter ajouté dans `AndroidManifest.xml` (scheme `io.supabase.baligh`, host `login-callback`). (3) `authOptions.redirectTo` configuré dans `Supabase.initialize()` pour la gestion automatique du deep link. Le lien va désormais vers l'app au lieu de localhost. ⚠️ **Dashboard requis :** ajouter `io.supabase.baligh://login-callback` dans Supabase → Authentication → Settings → Redirect URLs. |
